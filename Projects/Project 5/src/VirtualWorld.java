   ...
   extends PApplet
{
   ...
   */
   ...
   //Localize mouse click
   private Point mouseToPoint(int x, int y, Viewport viewport)
   {
      return new Point((x/(TILE_WIDTH)+viewport.getColDelta()), (y/TILE_HEIGHT)+viewport.getRowDelta());
   }

   public void mousePressed(){
      Point clickedPoint = mouseToPoint(mouseX, mouseY, this.view.getViewport());
      if (!world.isOccupied(clickedPoint) && Ray.RayPopulation < Ray.RAY_POPULATION_LIMIT){
         RayNotFull newRay = RayNotFull.createRayNotFull("", clickedPoint, this.imageStore.getImageList(Ray.RAY_KEY), 1, Ray.RAY_ACTION_PERIOD, Ray.RAY_ANIMATION_PERIOD);
         world.addEntity(newRay);
         newRay.scheduleAction(this.scheduler, this.world, this.imageStore);

         //Set Point Clicked to sand
         if(world.getBackgroundImage(clickedPoint).get().equals(imageStore.getImageList("sea").get(0))){
            world.setBackground(clickedPoint, new Background("sand", imageStore.getImageList("sand")));
         }

         //Set neighbors to sand
         MovableEntity.DIAGONAL_CARDINAL_NEIGHBORS.apply(clickedPoint).forEach(point -> {
            if(world.withinBounds(point) && world.getBackgroundImage(point).get().equals(imageStore.getImageList("sea").get(0))){
